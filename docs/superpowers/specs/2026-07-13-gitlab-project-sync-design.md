...
## Configuration
`--host` selects the GitLab hostname and `--namespace` selects the recursively enumerated GitLab group; both default to `git.iserv.eu` and `iserv`. `--workspace` selects the local checkout root and defaults to `~/git/iserv`.
